...
/**
 * CLOVA.WORLD_CITY インテントハンドラ
 */
export const SlotWorldCityIntentHandler: RequestHandler = {
  /**
   * 実行判定
  ...
  canHandle(handlerInput: Clova.Context) {
    return (
      handlerInput.getIntentName() === 'SlotWorldCityIntent'
    );
  },
  /**
   * ハンドラ実行
  ...
  handle(handlerInput: Clova.Context) {
    ...
    // スロット収集
    const customSlot = handlerInput.getSlot('worldCity');
