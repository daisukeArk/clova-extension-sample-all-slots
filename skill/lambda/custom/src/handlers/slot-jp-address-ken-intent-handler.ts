...
/**
 * CLOVA.JP_ADDRESS_KEN インテントハンドラ
 */
export const SlotJpAddressKenIntentHandler: RequestHandler = {
  /**
   * 実行判定
  ...
  canHandle(handlerInput: Clova.Context) {
    return (
      handlerInput.getIntentName() === 'SlotJpAddressKenIntent'
    );
  },
  /**
   * ハンドラ実行
  ...
  handle(handlerInput: Clova.Context) {
    ...
    // スロット収集
    const customSlot = handlerInput.getSlot('jpAddressKen');
