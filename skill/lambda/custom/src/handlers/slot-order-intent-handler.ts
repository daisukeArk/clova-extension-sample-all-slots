...
/**
 * CLOVA.ORDER インテントハンドラ
 */
export const SlotOrderIntentHandler: RequestHandler = {
  /**
   * 実行判定
  ...
  canHandle(handlerInput: Clova.Context) {
    return (
      handlerInput.getIntentName() === 'SlotOrderIntent'
    );
  },
  /**
   * ハンドラ実行
  ...
  handle(handlerInput: Clova.Context) {
    ...
    // スロット収集
    const customSlot = handlerInput.getSlot('order');
