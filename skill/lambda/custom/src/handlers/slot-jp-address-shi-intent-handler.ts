...
/**
 * CLOVA.JP_ADDRESS_SHI インテントハンドラ
 */
export const SlotJpAddressShiIntentHandler: RequestHandler = {
  /**
   * 実行判定
  ...
  canHandle(handlerInput: Clova.Context) {
    return (
      handlerInput.getIntentName() === 'SlotJpAddressShiIntent'
    );
  },
  /**
   * ハンドラ実行
  ...
  handle(handlerInput: Clova.Context) {
    ...
    // スロット収集
    const customSlot = handlerInput.getSlot('jpAddressShi');
