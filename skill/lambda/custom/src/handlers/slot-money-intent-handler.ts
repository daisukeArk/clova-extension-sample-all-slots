...
/**
 * CLOVA.MOENY インテントハンドラ
 */
export const SlotMoneyIntentHandler: RequestHandler = {
  /**
   * 実行判定
  ...
  canHandle(handlerInput: Clova.Context) {
    return (
      handlerInput.getIntentName() === 'SlotMoneyIntent'
    );
  },
  /**
   * ハンドラ実行
  ...
  handle(handlerInput: Clova.Context) {
    ...
    // スロット収集
    const customSlot = handlerInput.getSlot('money');
